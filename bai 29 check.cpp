...
void ThongKeTanSoXuatHien(char* tenFile) {	
	...
	while (1)	{		
		...
		huffTree[c].freq ++; // Ghi chú: Thao tác này d? làm gì (nghia là gì)?		
	}
...
void XuatBangThongKe() {
	...
	for (int i = 0; i < 256; i++) {
		if (huffTree[i].freq > 0) {// Ghi chú: t?i sao c?n ki?m tra di?u ki?n này?
			printf("%c: %d\n",i, huffTree[i].freq);
		}
	}
}
...
// tim 2 phan tu co trong so nho nhat
	for (int k = 0; k < nNode; k++)
		if (huffTree[k].used == false && huffTree[k].freq > 0)		{ // Ghi chú: T?i sao c?n ki?m tra 2 di?u ki?n này
			if (i == -1) {
				i = k;
			}
			else if (j == -1) {
...
int TaoCayHuffman() {
	...
	return nNode - 1; // Ghi chú: ý nghia c?a giá tr? tr? v??
...
void DuyetCayHuffman(int node, char maBit[], int nMaBit) {
	...
	if (huffTree[node].nLeft == -1 && huffTree[node].nRight == -1) {	//Ghi chú: ý nghia c?a di?u ki?n ki?m tra là gì?	
		...
		//Ghi chú: ý nghia c?a 2 dòng l?nh bên du?i là gì?
		maBit[nMaBit] = 0;		
...
void PhatSinhMaBit(int nRoot) { // Ghi chú: ý nghia c?a tham s? nRoot?
	for (int i = 0; i < 256; i++) {
		bangMaBit[i].soBit = 0;
...
void XuatByte(unsigned char out, int soBitCoNghia) {
	for (int i = 7; i >= 7 - soBitCoNghia + 1; i--) {
		if ((out >> i) & 1) {// Ghi chú: Thao tác này d? làm gì?
			printf("1");
		}
		else {
			printf("0");		
...
void MaHoa1KyTu(unsigned char c, unsigned char &out, unsigned char &viTriBit) { //Ghi chú: ý nghia c?a các tham s? truy?n vào?

	for (int i = 0; i < bangMaBit[c].soBit; i++) {
		if (bangMaBit[c].bits[i] == 1) {
			out = out | (1 << viTriBit); // Ghi chú: Thao tác này d? làm gì?
		}
